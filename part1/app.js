...
var mysql = require('mysql2/promise');var app = express();app.use(logger('dev'));

app.use(express.json());
app.use(express.urlencoded({ extended: false }));

app.use(cookieParser());let db;(async () => {
  try {
...
app.get('/api/walkers/summary', async (req, res) => {  try {
    const [summary] = await db.query(      SELECT         u.username AS walker_username,         COUNT(DISTINCT wr.rating_id) AS total_ratings,         ROUND(AVG(wr.rating), 1) AS average_rating,         COUNT(DISTINCT wa.request_id) AS completed_walks       FROM Users u       LEFT JOIN WalkRatings wr ON wr.walker_id = u.user_id       LEFT JOIN WalkApplications wa         ON wa.walker_id = u.user_id         AND wa.status = 'accepted'         AND EXISTS (           SELECT 1 FROM WalkRequests r           WHERE r.request_id = wa.request_id           AND r.status = 'completed'         )       WHERE u.role = 'walker'       GROUP BY u.user_id    );
    res.json(summary);
  } catch (err) {
    console.error(err);
